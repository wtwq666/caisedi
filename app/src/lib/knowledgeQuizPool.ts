...
export function getModuleStatsForTag(source: QuizSource, tagKey: string): QuizModuleStat[] {
  ...
  return order
    .map((m) => ({
      key: m.key,
      label: m.label,
      count: grouped.get(m.key)?.count ?? 0,
    }))
    .filter((m) => m.count > 0)
}
...
export function getPoolStats() {
  return {
    fabric: fabricPool.length,
    product: productPool.length,
  }
}
...
export function scoreFeedback(source: QuizSource, percent: number): string {
  if (source === 'product') {
    if (percent >= 90) return '本系列识款能力优秀，已能看图对应各板块要点。'
